...
### 34. [2026-08-20] 最终配置矩阵（32GB 实测）
| 档位 | 配置 | 显存 | 图片输入 | MTP |
|------|------|------|---------|-----|
| 262K 全功能（默认） | `-c 262144` + MTP + mmproj | 27.7GB | ✅ | ✅ |
| 512K + 视觉 | YaRN + mmproj（无 MTP） | 30.4GB | ✅ | ❌ |
| 512K + MTP + mmproj | 三全开 | >32GB OOM | - | - |
| 1M | YaRN factor 4 | >38GB ❌ | - | - |

→ 默认档选 262K 全功能（视觉 + MTP + 256K 以上上下文全部满足）；
  512K 档适合超长上下文场景，代价是慢 30-40% 且无推测解码。
