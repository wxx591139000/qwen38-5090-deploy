...
curl -sf "$BASE_URL/v1/chat/completions" \
  ...
  | python3 -c "import sys,json; d=json.load(sys.stdin); print('回复:', d['choices'][0]['message']['content'][:200])" \
...
curl -sf "$BASE_URL/v1/responses" \
  ...
  | python3 -c "import sys,json; d=json.load(sys.stdin); print('回复:', d.get('output_text','')[:200])" \
